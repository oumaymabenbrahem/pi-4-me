...
function ShoppingProductTile({
  ...
  return (
    <Card className="w-full max-w-sm mx-auto overflow-hidden flex flex-col h-full shadow-md card-hover rounded-xl">
      <div className="flex-1 flex flex-col">
        ...
        >
          ...
          {product?.quantity === 0 ? (
            ...
            <Badge className="absolute top-3 left-3 bg-orange-500 hover:bg-orange-600 shadow-md">
              {`Plus que ${product?.quantity} ${product?.unit}`}
            </Badge>
          ) : isCloseToExpiration() ? (
            <Badge className="absolute top-3 left-3 bg-yellow-500 hover:bg-yellow-600 shadow-md">
              Expire bientôt
            </Badge>
          ) : null}

          {product?.isCollected && (
            <Badge className="absolute top-3 right-3 bg-green-500 hover:bg-green-600 shadow-md">
              Collecté
            </Badge>
          )}

          <div className="absolute bottom-0 left-0 right-0 p-3 text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
            <span className="text-sm font-medium">Cliquez pour voir les détails</span>
          </div>
        </div>
        <CardContent
        ...
        >
          <h2 className="text-lg font-semibold mb-3 truncate">{product?.title}</h2>
          <div className="flex flex-col space-y-3">
            <div className="flex justify-between items-center">
              ...
              <Badge variant="outline" className="text-xs bg-secondary/10 text-secondary border-secondary/20 hover:bg-secondary/20">
                {brandOptionsMap[product?.brand]}
              </Badge>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="flex items-center gap-1">
                <span className="font-medium">Qté:</span> {product?.quantity} {product?.unit}
              </span>
              <span className="flex items-center gap-1">
                <span className="font-medium">Exp:</span> {formatDate(product?.expirationDate)}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-lg font-bold text-primary">{product?.price || 0} DT</span>
            </div>
